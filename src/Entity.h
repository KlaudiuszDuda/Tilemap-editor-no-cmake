...
public:
	...
	inline void computeModelMatrix(const glm::mat4& parentGlobalModelMatrix)
	{
		m_modelMatrix = parentGlobalModelMatrix * getLocalModelMatrix();
		m_isDirty = false;
	}

	inline void setLocalPosition(const glm::vec3& newPosition)
	{
		m_pos = newPosition;
		m_isDirty = true;
	}

	inline void setLocalRotation(const glm::vec3& newRotation)
	{
		m_eulerRot = newRotation;
		m_isDirty = true;
	}

	inline void setLocalScale(const glm::vec3& newScale)
	{
		m_scale = newScale;
		m_isDirty = true;
	}
	...
	inline const glm::vec3& getLocalRotation() const
	{
		return m_eulerRot;
	}
...
struct AABB : public BoundingVolume
{
	...
	inline std::array<glm::vec3, 8> getVertice() const
	{
		std::array<glm::vec3, 8> vertice;
		vertice[0] = { m_Center.x - m_Extents.x, m_Center.y - m_Extents.y, m_Center.z - m_Extents.z };
		vertice[1] = { m_Center.x + m_Extents.x, m_Center.y - m_Extents.y, m_Center.z - m_Extents.z };
		vertice[2] = { m_Center.x - m_Extents.x, m_Center.y + m_Extents.y, m_Center.z - m_Extents.z };
		vertice[3] = { m_Center.x + m_Extents.x, m_Center.y + m_Extents.y, m_Center.z - m_Extents.z };
		vertice[4] = { m_Center.x - m_Extents.x, m_Center.y - m_Extents.y, m_Center.z + m_Extents.z };
		vertice[5] = { m_Center.x + m_Extents.x, m_Center.y - m_Extents.y, m_Center.z + m_Extents.z };
		vertice[6] = { m_Center.x - m_Extents.x, m_Center.y + m_Extents.y, m_Center.z + m_Extents.z };
		vertice[7] = { m_Center.x + m_Extents.x, m_Center.y + m_Extents.y, m_Center.z + m_Extents.z };
		return vertice;
	}
	...
	{
		//Get global scale thanks to our transform
		const glm::vec3 globalCenter{ transform.getModelMatrix() * glm::vec4(m_Center, 1.f) };

		// Scaled orientation
		const glm::vec3 right = transform.getRight() * m_Extents.x;
		const glm::vec3 up = transform.getUp() * m_Extents.y;
		const glm::vec3 forward = transform.getForward() * m_Extents.z;
		...
		AABB globalAABB;
		globalAABB.set(globalCenter, { newIi, newIj, newIk });
...
inline std::array<glm::vec3, 8> getAABBVertice(glm::vec3 center, glm::vec3 extents)
{
	std::array<glm::vec3, 8> vertice;
...
inline bool isAABBOnFrustum(AABB aabb, const Frustum& camFrustum, const Transform& transform)
{
	//Get global scale thanks to our transform
	const glm::vec3 globalCenter{ transform.getModelMatrix() * glm::vec4(aabb.m_Center, 1.f) };

	// Scaled orientation
	const glm::vec3 right = transform.getRight() * aabb.m_Extents.x;
	const glm::vec3 up = transform.getUp() * aabb.m_Extents.y;
	const glm::vec3 forward = transform.getForward() * aabb.m_Extents.z;
