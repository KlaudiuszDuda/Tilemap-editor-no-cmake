...
TilemapRender::TilemapRender()
{
	...
	textureTileData.init(tilemapChunkSizeX * tilemapChunkSizeY * CHUNK_SIZE_SQUARED * sizeof(u32));
	...
	std::fill(dst, tilemapChunkSizeX * tilemapChunkSizeY * CHUNK_SIZE_SQUARED + dst, 1u);
...
void TilemapRender::updateCanvasEdit()
{
	...
	{
		f32 mouseWorldPositionX = window.getMousePosition().x + camera.Position.x;
		f32 mouseWorldPositionY = window.getMousePosition().y + camera.Position.y;
	}
}
...
void TilemapRender::draw()
{
	...
	for (i32 i = 0; i < tilemapChunkSizeX * tilemapChunkSizeY; i++)
	{
		...
		if (isAABBOnFrustum(setAABB({ x * CHUNK_SIZE, y * CHUNK_SIZE, 0 }, { (x + 1) * CHUNK_SIZE, (y + 1) * CHUNK_SIZE, 0 }), camFrustum))
		{
			...
			textureVertexArray.VertexAttribIPointer(0, 1, GL_UNSIGNED_INT, sizeof(u32), (void*)(i * CHUNK_SIZE_SQUARED * sizeof(u32)));
