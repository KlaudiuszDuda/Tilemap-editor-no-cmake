...
void ImGui_ImplGlfw_InstallCallbacksAdapter(GLFWwindow* window)
{
    ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData(window);
    IM_ASSERT(bd->InstalledCallbacks == false && "Callbacks already installed!");
    IM_ASSERT(bd->Window == window);
    ...
    bd->PrevUserCallbackWindowFocus = glfwSetWindowFocusCallback(window, ImGui_ImplGlfw_WindowFocusCallback);
    bd->PrevUserCallbackCursorEnter = glfwSetCursorEnterCallback(window, ImGui_ImplGlfw_CursorEnterCallback);
    bd->PrevUserCallbackCursorPos = glfwSetCursorPosCallback(window, Callback::windowMousePositionEvent); // windowMousePositionEvent
    bd->PrevUserCallbackMousebutton = glfwSetMouseButtonCallback(window, Callback::updateMouseButtonCallback); // updateMouseButtonCallback
    bd->PrevUserCallbackScroll = glfwSetScrollCallback(window, Callback::updateScrollWheelCallback); // updateScrollWheelCallback
    bd->PrevUserCallbackKey = glfwSetKeyCallback(window, Callback::updateKeyboardCallback); // updateKeyboardCallback
    bd->PrevUserCallbackChar = glfwSetCharCallback(window, ImGui_ImplGlfw_CharCallback);
    bd->PrevUserCallbackMonitor = glfwSetMonitorCallback(ImGui_ImplGlfw_MonitorCallback);
    bd->InstalledCallbacks = true;
}
